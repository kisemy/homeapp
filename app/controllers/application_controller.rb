...
class ApplicationController < ActionController::Base
  ...
  def get_clients_and_items
    ...
    @get_items = get_items(parent)
     #@get_glentries= get_glentries(parent)
 ...
 #def get_glentries(parent)
   # options = ''
    #parent.glentries.each { |glentry| options += "<option value=#{glentry.id} type='company_level'>#{client.organization_name}</option>" } if parent.glentries.present?
  #  options
  #end
