...
RSpec.describe "morgages/edit", :type => :view do
  before(:each) do
    @morgage = assign(:morgage, Morgage.create!(
      :Firstname => "MyText",
      :Middlename => "MyText",
      :Lastname => "MyText",
      :IDno => "MyText",
  ...
  it "renders the edit morgage form" do
    render

    assert_select "form[action=?][method=?]", morgage_path(@morgage), "post" do
